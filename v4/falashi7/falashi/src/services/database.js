...
export async function initDatabase() {
  ...
  await db.execAsync(`
    ...
    CREATE TABLE IF NOT EXISTS equipped (
      id INTEGER PRIMARY KEY NOT NULL,
      cosmetic_id TEXT NOT NULL DEFAULT 'avatar_default'
    );

    INSERT OR IGNORE INTO equipped (id, cosmetic_id) VALUES (1, 'avatar_default');
  `);
...
export async function fetchEquippedCosmetic() {
  const database = getDb();
  const row = await database.getFirstAsync('SELECT cosmetic_id FROM equipped WHERE id = 1');
  return row?.cosmetic_id ?? 'avatar_default';
}

export async function equipCosmetic(cosmeticId) {
  const database = getDb();
  await database.runAsync('UPDATE equipped SET cosmetic_id = ? WHERE id = 1', cosmeticId);
}
